defmodule Poker.RankTest do
  ...
  test "compare - high card - tie" do
    ...
    assert Rank.compare(left, right) == %Rank{winner: 0, category: "C", card: nil}
  end

  test "compare - high card - left" do
  ...
  test "compare - three of a kind - left, cat" do
    ...
    assert  Rank.compare(left, right) == %Rank{winner: 1, category: "3", card: nil}
  end
  ...
  test "compare - three of a kind - left, card" do
    ...
    assert {:ok, card}  = Card.new("6H")
    assert  Rank.compare(left, right) == %Rank{winner: 1, category: "3", card: card}
  end

  test "compare - straight - tie" do
    assert {:ok, left}  = Category.new("6H 4C 7S 3H 5H")
    assert {:ok, right} = Category.new("6C 7H 4S 3C 5C")
    assert  Rank.compare(left, right) == %Rank{winner: 0, category: "S", card: nil}
  end
  ...
  test "compare - full house - left, card" do
    ...
    assert {:ok, card}  = Card.new("9C")
    assert  Rank.compare(left, right) == %Rank{winner: 1, category: "H", card: card}
  end
  ...
  test "compare - 4 of a kind - left, card" do
    ...
    assert {:ok, card}  = Card.new("9C")
    assert  Rank.compare(left, right) == %Rank{winner: 1, category: "4", card: card}
  end

  test "compare - straight flush - left, cat" do
    assert {:ok, left}  = Category.new("7H 6H 4H 3H 5H")
    assert {:ok, right} = Category.new("8C 8H 8S 8D 5D")
    assert  Rank.compare(left, right) == %Rank{winner: 1, category: "T", card: nil}
  end
